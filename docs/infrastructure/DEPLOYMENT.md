...
3. **Kubernetes** - для масштабируемого развертывания (см. [ROADMAP.md](ROADMAP.md))
...
```yaml
services:
  # Автоматические миграции (init container)
  migrate:
    image: migrate/migrate
    command: ["-path=/migrations", "-database", "${DB_DSN}", "up"]

  # FTP сервер (опционально, для тестирования)
  ftp-server:
    image: fauria/vsftpd
    ports:
      - "${FTP_PORT}:${FTP_PORT}"

  # Webhook server (основной сервис)
  webhook-server:
    build: .
    ports:
...
См. [ROADMAP.md](ROADMAP.md) для планов интеграции с Prometheus.
