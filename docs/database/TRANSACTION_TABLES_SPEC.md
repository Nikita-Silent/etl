...
- Поле `operation_type` (№13) может быть `0` (продажа).
...
| 32 | item_type_code | INT | Тип номенклатуры (0 — обычный товар) |
...
- поле №32: тип номенклатуры, `0` — обычный товар (см. PDF).
...
Особенности (по PDF):
- поле №10: тип цены (`0` — спеццена, `1` — цена из прайс-листа).
