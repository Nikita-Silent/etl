...
Таблица фиксирует успешную транзакционную загрузку логического файла Frontol в БД.
Используется для защиты от повторной загрузки после успешного DB commit и для reconcile исправленных переотгрузок в рамках одной и той же даты.
...
Таблица хранит durable lifecycle ETL-операции целиком: от приема webhook/download/CLI запуска до финального статуса.
Нужна для трассировки по `operation_id` и для обнаружения незавершенных операций после рестарта процесса.
